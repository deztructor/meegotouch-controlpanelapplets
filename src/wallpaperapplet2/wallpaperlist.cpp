...
{
    /*
     * Funy thing, sometimes we get this signal while the list is panning. It is
     * very annoying for the user, so we just filter these signals here.
     */
    if (m_Panning) {
        SYS_WARNING ("+++ Dropping the event, we are panning.");
        return;
    }
